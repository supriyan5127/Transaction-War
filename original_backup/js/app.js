...
document.getElementById('form-transfer').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const data = await res.json();
        if (data.status === 'success') {
            document.getElementById('modal-success-title').textContent = 'Transfer Completed';
    ...
    try {
        ...
        if (data.status === 'success' && data.data.length > 0) {
            data.data.forEach(user => {
                const avatar = user.profile_image ? user.profile_image : getDefaultAvatar(user.username);
                const html = `
                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 1rem; border-bottom: 1px solid var(--border-color);">
                        ...
                        <div style="display:flex; gap:10px;">
                            ...
                            <button onclick="prepTransfer('${user.username}')" class="btn" style="padding: 0.5rem 1rem; background-color: var(--success);">Send</button>
                        </div>
                    </div>
                `;
